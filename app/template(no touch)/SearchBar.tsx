    ...
    <div className="search__background">
      <div className="search__wrapper">
        <figure style={{ display: "flex", alignItems: "center" }}>
          <Image src="/assets/logo.png" alt="Summarist" width={120} height={30} style={{ height: 30, width: "auto", objectFit: "contain" }} />
        </figure>
        <div className="search__content">
          ...
          <div className="sidebar__toggle--btn" onClick={() => setSidebarOpen(!sidebarOpen)}>
        ...
        {searchQuery && (
          <div className="search__books--wrapper">
            {searching && (
              <div style={{ padding: 16, color: "#6b757b", fontSize: 14 }}>Searching...</div>
            )}
            {!searching && searchResults.length === 0 && (
              <div style={{ padding: 16, color: "#6b757b", fontSize: 14 }}>No results found</div>
            )}
            {searchResults.map((book) => (
              <Link key={book.id} className="search__book--link" href={`/book/${book.id}`} onClick={() => setSearchQuery("")}>
